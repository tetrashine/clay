...
var Base = /*#__PURE__*/function (_Evented) {
  ...
  _createClass(Base, [{
    ...
    key: "appendChildByElement",
    value: function appendChildByElement(elem) {
    ...
    key: "removeChildByElement",
    value: function removeChildByElement(elem) {
...
var Board = /*#__PURE__*/function (_Base) {
  ...
  function Board(doc, dom, width, height, zoom, editable) {
    ...
    defs.innerHTML = "\n      <marker id='head' orient=\"auto\" markerWidth='2' markerHeight='4' refX='0.1' refY='2'><path d='M0,0 V4 L2,2 Z' fill=\"".concat(LINK_COLOR, "\"/></marker>\n      <marker id='head-selected' orient=\"auto\" markerWidth='2' markerHeight='4' refX='0.1' refY='2'><path d='M0,0 V4 L2,2 Z' fill=\"").concat(LINK_SELECTED_COLOR, "\"/></marker>\n    ");
  ...
  _createClass(Board, [{
    ...
    value: function initialize(editable) {
      ...
      var origin = undefined; //board drag

      this.elem.addEventListener('mousedown', function (evt) {
        if (event.which === 1) {
          //left click
          origin = getCoordsFromEvent(evt, _this2.elem);
      ...
      this.elem.addEventListener('mousemove', function (evt) {
        if (event.which == 1) {
          ...
          } else if (origin && _this2._nodes.every(function (_) {
            return !_.isDragging();
          })) {
            highlighting = true;
            var left = origin.x < point.x ? origin.x : point.x;
            var right = origin.x > point.x ? origin.x : point.x;
            var top = origin.y < point.y ? origin.y : point.y;
            var bottom = origin.y > point.y ? origin.y : point.y;
      ...
      this.elem.addEventListener('mouseup', function (evt) {
        if (highlighting) {
          ...
          _this2.elem.getIntersectionList(svgRect, null).forEach(function (_) {
            ...
            if (node) {
              node.select();
              node.on('drag', _this2.onNodeDrag.bind(_this2, node));
          ...
          _this2.reorderItems(_this2._selected);
    ...
    key: "onNodeDrag",
    value: function onNodeDrag(node, _ref) {
      var dx = _ref.dx,
          dy = _ref.dy;

      this._selected.filter(function (n) {
        return n !== node;
      }).forEach(function (n) {
        n.setXY(n.x + dx, n.y + dy);

        n._links.forEach(function (link) {
    ...
    key: "reorderItems",
    value: function reorderItems(selected) {
      var _this3 = this;

      selected.forEach(function (baseItem) {
        _this3.removeChild(baseItem);

        _this3.appendChild(baseItem);
      });
    }
  }, {
    key: "unselectItems",
    value: function unselectItems() {
      ...
      this._selected.forEach(function (_) {
        ...
        _.off('drag', _this4.onNodeDrag.bind(_this4, _));
      });
    ...
    value: function subscribeToSelection(item) {
      ...
      item.on('clickonly', function (e) {
        if (item.selected) {
          ...
          _this7._selected.splice(_this7._selected.indexOf(item), 1);

          _this7.trigger('ondeselect', _this7._selected);
        } else {
          _this7.unselectItems();

          item.select();

          _this7._selected.push(item);

          _this7.trigger('onselect', _this7._selected);

          _this7.reorderItems(_this7._selected);

          _this7.clearHighlight();
        }
      });
    }
  }, {
    ...
    value: function enterSelectionMode() {
      ...
      this._sel.onclick = function (evt) {
        var point = getCoordsFromEvent(evt, _this8._sel); //exit

        _this8.exitSelectionMode();
    ...
    value: function enterNodeMode() {
      ...
      this.enterSelectionMode(function (point) {
        var config = {
          x: point.x - _this9._transformMatrix[4],
          y: point.y - _this9._transformMatrix[5],
          title: 'New Title',
          editable: _this9._editable
        };
        var node = new displays_node__WEBPACK_IMPORTED_MODULE_0__["default"](_this9._doc, null, config);
    ...
    value: function enterLinkMode() {
      ...
      var onLinkConstructed = function onLinkConstructed(node, ioIndex) {
        if (_this10._link) {
          ...
          _this10._link.setTarget(node, ioIndex);

          _this10._link.setDotted(false);

          _this10._link.selectable(); //clean up


          onComplete();

          _this10._nodes.forEach(function (node) {
            node.startListening();
            node.removeLinkables();
            node.resetColor();
          });

          _this10._link = undefined;
        } else {
          var link = _this10._link = new displays_link__WEBPACK_IMPORTED_MODULE_1__["default"](_this10._doc, node, ioIndex, new displays_mouse__WEBPACK_IMPORTED_MODULE_3__["default"](_this10.elem.getBoundingClientRect()), -1, {
            dotted: true,
            editable: _this10._editable
          });

          _this10.addLink(link);
    ...
    value: function enterZoomMode(type) {
      ...
      this._sel.onclick = function (evt) {
        var scale = _this11._scale + (type === constants_zoommode__WEBPACK_IMPORTED_MODULE_5__["ZoomMode"].ZoomIn ? 1 : -1) * 0.25;
        var point = getCoordsFromEvent(evt, _this11._sel);

        _this11.zoom(scale, point);
      };
    }
  }, {
    key: "exitZoomMode",
    ...
    value: function buildLinkablesByNodes(doc, nodes, onComplete) {
      ...
      var linkables = nodes.map(function (node) {
        ...
        var linkables = coordsList.map(function (coords) {
          return _this12.generateLinkable(doc, coords, node, onComplete);
        });
    ...
    value: function generateLinkable(doc, coords, node, onComplete) {
      ...
      div.onclick = function () {
        ...
        if (_this13._linked) {
          _this13._mode = constants_editmode__WEBPACK_IMPORTED_MODULE_4__["EditMode"].None; //complete linking target

          _this13._linked._link.setTarget(node, div.getAttribute('index'));

          _this13._linked._link.setDotted(false);

          _this13._linked._link.selectable();

          _this13._linked._link = undefined;
          _this13._linked.selected = false;
          _this13._linked.style.backgroundColor = NODE_CONNECTOR_COLOR;
          _this13._linked = undefined; //exit

          _this13.exitLinkMode();
          ...
          var link = new displays_link__WEBPACK_IMPORTED_MODULE_1__["default"](_this13._doc, node, index, new displays_mouse__WEBPACK_IMPORTED_MODULE_3__["default"](_this13.elem.offsetLeft, _this13.elem.offsetTop), -1, {
            dotted: true,
            editable: _this13._editable
          });
          div._link = link; //add to links

          _this13.addLink(link);
        }
      };
...
var Draggable = /*#__PURE__*/function (_Base) {
  ...
  _createClass(Draggable, [{
    ...
    value: function startListening() {
      ...
      this.elem.addEventListener('touchstart', this.startDragFn, {
        passive: false
      });
    }
  }, {
    ...
    value: function stopListening() {
      ...
      this.elem.removeEventListener('touchstart', this.startDragFn, {
        passive: false
      });
    }
  }, {
...
var Node = /*#__PURE__*/function (_Draggable) {
  ...
  _createClass(Node, [{
    ...
    key: "addLink",
    value: function addLink(link, type) {
      if (type === 'input') {
    ...
    value: function deleteLink(link) {
      var inputIndex = this._inputLinks.indexOf(link);

      var outputIndex = this._outputLinks.indexOf(link);
    ...
    value: function generateTable(perm, attrs) {
      ...
      var table = displays_graphics_svgtable__WEBPACK_IMPORTED_MODULE_2__["default"].generate(this._doc, ['Attribute', 'Value', "<button class=\"icon-btn add-btn\">".concat(displays_svg__WEBPACK_IMPORTED_MODULE_1__["Add"], "</button")], [].concat(_toConsumableArray(perm), _toConsumableArray(this.appendWithDeleteBtn(this.toEditable(attrs)))));
    ...
    key: "setXY",
    value: function setXY(x, y) {
      _get(_getPrototypeOf(Node.prototype), "setXY", this).call(this, x, y);
    ...
    key: "_setXY",
    value: function _setXY(x, y) {
      this.x = this._config.x = x;
      this.y = this._config.y = y;
      this.elem.setAttributeNS(null, 'transform', "translate(".concat(x, ",").concat(y, ")"));
    }
  }, {
    key: "addXY",
